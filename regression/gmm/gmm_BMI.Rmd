...
results_version <- '241223'
##################################################################################################################################
# Source the helper functions
source(here::here('bmi_paper_model_fitting_helper_functions_241223.R')) # NOTE THE VERSION, make sure it's the right one!
##################################################################################################################################



# reads 'data' 
load("/Volumes/research/metabogut/BMI_2021/analysisv3.0/regression/gmm/data_gmm_CLRtrans.RData")

# Since the regression model does not accept spaces in the variables names being regressed I will take the gmm ID only and run the regression model with it
...
for(curr_model in model_names){
    ...
    curr_results <- get_results_for_bmi_paper(d = data,
                                              ...
                                              gmm_sex_interaction = model_with_interaction[[curr_model]],
                                              sex_var = 'male')
...
interaction_df <- all_results_main %>%
    filter(grepl("int", model_name) & grepl(":maleTRUE", gmm))
...
interaction_df <- interaction_df[,c("analysis", "outcome","cohort","model_name","gmm","n_obs","est","se","ci_low","ci_high",
                      "p", "p_value_adj")]
...
results$outcome <- "BMI"
results <- results[,c("analysis", "outcome","cohort","model_name","gmm","n_obs","est","se","ci_low","ci_high",
                      "p", "p_value_adj")]
...
## save
```{r}
export(all_results, "/Volumes/research/metabogut/BMI_2021/analysisv3.0/regression/gmm/gmm_as_exposure/results/results_241223.xlsx")
```
