...
get_model_formula_helper <- function(outcome,                      # string
  ...
  if(base_model){
    vars_to_adjust_for <- c(gmm, base_model_vars_to_adjust_for)
  }else{
    vars_to_adjust_for <- c(gmm, full_model_vars_to_adjust_for)
  }
  ...
  if(gmm_sex_interaction){
    model_formula <- paste0(model_formula, ' + ', gmm, ':', sex_var)
  }
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# input:
# d = name of the dataset with samples in rows, gmms and other covariates in columns
...
# as it runs, it prints each of the 6 models as well as the estimates for the models for the first gmm.
...
get_results_for_bmi_paper <- function(d,       # dataset must be specified
  ...
  # get the names of all gmm's in the dataset
  all_gmm <- names(d)[grep('MF', names(d))]
  n_gmm = length(all_gmm)
  
  # 6-10 depending on the bmi_adj_for_waist and bmi_adj_for_WHR flags
  nrows <- n_gmm * (length(outcome)) * 
    (1 + ifelse(is.na(full_model_vars_to_adjust_for), 0, 1)) * 
    (1 + gmm_sex_interaction)
  
  results_ix <- 1:(1 + gmm_sex_interaction) # 1:1 or 1:2, the initial index for the results in the results matrix
  index_increment <- 1 + gmm_sex_interaction  # 1 or 2
  
  
  # make a 'dummmy' matrix for the results
  results_columns <- c('model_name', # model name
                       'gmm',   # name of the gmm, or gmm:male, etc
  ...
  for(base_model in c(T, F)){
    ...
    for (i in 1:n_gmm){
      ...
      curr_model_formula <- get_model_formula_helper(outcome = outcome, 
                                                     ...
                                                     gmm_sex_interaction = gmm_sex_interaction,
                                                     sex_var = sex_var)
      ...
      # print the model for the first gmm, just to verify that we're fitting the models we wanted
      ...
      if(print_curr_model & length(grep('plate', curr_model_formula)) == 0){
        ...
        cat(' \nModel estimates for the first gmm:\n') 
      ...
      est_of_interest_row_ix <- grep(all_gmm[i], names(curr_model$coeff)) # this will be wrong if NA's in the model
      ...
      results[results_ix, 'gmm'] <- names(curr_model$coeff)[est_of_interest_row_ix] # this works even if est is NA
      ...
      #          if, for example, the model doesn't fit for a given gmm)
