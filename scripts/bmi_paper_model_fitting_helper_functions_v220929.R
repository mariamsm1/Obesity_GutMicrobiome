...
# full_model_vars_to_adjust_for = c('physical_activity', 'metformin', 'PPI', 'Fiber')
#
#
# output:
...
get_results_for_bmi_paper <- function(d,       # dataset must be specified
                                      ...
                                      full_model_vars_to_adjust_for = c('physical_activity', 'metformin', 'PPI', 'Fiber'), # can be NA
                                      glm_family = 'gaussian',
    ...
    all_mgs <- names(d)[grep('HG3A', names(d))]
