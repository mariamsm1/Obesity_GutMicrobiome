# Marlena Maziarz 
# June 8, 2022, edited by Mariam Miari August 18, 23, 30, September 29, 2022
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# input:
# d = name of the dataset with samples in rows, kos and other covariates in columns
...
# as it runs, it prints each of the 6 models as well as the estimates for the models for the first ko.
...
get_results_for_bmi_paper <- function(d,       # dataset must be specified
  ...
  # get the names of all ko's in the dataset
  ...
  for(base_model in c(T, F)){
    ...
    for (i in 1:n_ko){
      ...
      # print the model for the first ko, just to verify that we're fitting the models we wanted
      ...
      #          if, for example, the model doesn't fit for a given ko)
