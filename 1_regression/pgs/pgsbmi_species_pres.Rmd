...
data_trans <- read_excel("/Volumes/research/metabogut/BMI_2021/data/processed/MGS2.0/data_nds_filt_CLRtrans_9993_updated250208.xlsx")
...
# fit the model 
bmi_analysis <- function(formula, data, mgs_each, model_name, analysis, exposure, interaction_term = NULL) {
...
for (mgs_each in mgs_trans) {
  ...
  results <- rbind(results, bmi_analysis(formula_full, subset_df, mgs_each, paste("full_M1", prs_bmi, sep = "_"), "all",prs_bmi))

  # stratified models for males
  results <- rbind(results, bmi_analysis(as.formula(paste(mgs_each, "~", prs_s_bmi, "+", paste(confounders_full, collapse = "+"))), subset_df %>% filter(male == TRUE), mgs_each, paste("full_stratM1", prs_s_bmi, "males", sep = "_"), "sex_stratification",prs_s_bmi))

  # stratified models for females
  results <- rbind(results, bmi_analysis(as.formula(paste(mgs_each, "~", prs_s_bmi, "+", paste(confounders_full, collapse = "+"))), subset_df %>% filter(male == FALSE), mgs_each, paste("full_stratM1", prs_s_bmi, "females", sep = "_"), "sex_stratification",prs_s_bmi))
  ...
  results <- rbind(results, bmi_analysis(formula_interaction_full, subset_df, mgs_each, paste("full_intM1", prs_s_bmi, sep = "_"), "interaction", prs_s_bmi, interaction_term = interaction_term))
}
