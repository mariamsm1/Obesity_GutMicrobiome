#Mariam Miari
#250604
# variance explained by BMI in diversity metrics
...
pheno <- merged_df$BMI  # bmi is now the predictor
...
var.fun <- function(pheno, tax, metric) {
  ...
  cv_folds <- createFolds(data$diversity_metric, k = 10)
  ...
  cv <- lapply(cv_folds, function(cv_fold) {
    ...
    # full model (with BMI as predictor)
    fit_full <- glm(diversity_metric ~ pheno + age + sex, data = train, family = gaussian())
    
    # null model (without BMI)
    fit_null <- glm(diversity_metric ~ age + sex, data = train, family = gaussian())
    
    # prediction on test set only
    ...
    r.squared.full.test <- cor(pred_full_test, test$diversity_metric)^2
    r.squared.null.test <- cor(pred_null_test, test$diversity_metric)^2
    
    # R² difference (contribution of BMI)
...
# define the diversity metrics to test
...
for(i in 1:n_boot){
  ...
  boot_ix <- sample.int(n, replace = TRUE)  # sample with replacement
...
# export results
write.xlsx(var_exp_all, file = "results/VE_by_BMI_with_CI.xlsx")
